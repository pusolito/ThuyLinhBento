...
import com.thuylinh.bento.adapter.ProductImageAdapter
import com.thuylinh.bento.adapter.RecyclerViewAdapter
import com.thuylinh.bento.models.ProductModelNew
import com.thuylinh.bento.models.RequestModel
import com.thuylinh.bento.utils.Constants.KeyIntent.IS_ADDED_TO_CART
...
class ProductDetailActivityNew : AppBaseActivity() {
    ...
    private fun onFavouriteClick(image: ImageView) {
        if (isExistInWishList(mProductModel?.id!!)) {
            changeFavIcon(image, R.drawable.ic_heart, R.color.gray_80); ivFavourite.isClickable = false
            val requestModel = RequestModel(); requestModel.pro_id =
